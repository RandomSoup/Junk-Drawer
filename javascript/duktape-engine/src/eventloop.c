...
#define DUK_FILE_MACRO "eventloop.c"
...
static duk_ret_t eventloop_run_raw(duk_context *ctx, void *udata) {
    (void) udata;
    ...
    duk_compile_string(ctx, 0, duk_get_string(ctx, -2));
    duk_call(ctx, 1);
    duk_pop(ctx);
...
int eventloop_register(duk_context *ctx) {
    efd = eventfd(0, 0);
...
int eventloop_run(duk_context *ctx, char *src) {
    duk_push_string(ctx, src);
    if (duk_safe_call(ctx, eventloop_run_raw, NULL, 1 /*nargs*/, 1 /*nret*/) != 0) {
        return 1;
    }
    return 0;
}
